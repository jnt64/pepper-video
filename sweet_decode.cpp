...
#undef NDEBUG // This is for assert. Remove.
...
#define SWEET_ASSERT_GL_THROW(ppOpenGLES2Ptr, ppGraphics3DPtr)          \
...
class Decoder;
class SweetDecodeInstance;

struct PendingPicture 
{
     PendingPicture (Decoder* decoder, const PP_VideoPicture& picture) : decoder(decoder), picture(picture) {}
    ~PendingPicture() {}
...
class SweetDecodeInstance : public pp::Instance,
                            public pp::Graphics3DClient 
{
public:
             SweetDecodeInstance (PP_Instance instance, pp::Module* module);
    virtual ~SweetDecodeInstance ();
    ...
    virtual void Graphics3DContextLost () {SWEET_ASSERT_THROW(false, "Graphics3DContextLost");}
...
private:
    ...
    {
    public:
         LogError (SweetDecodeInstance* instance) : instance_(instance) {}
        ~LogError ()
         {
    ...
    pp::CompletionCallbackFactory<SweetDecodeInstance> callback_factory_;
...
{
public:
     Decoder (SweetDecodeInstance* instance, int id, const pp::Graphics3D& graphics_3d);
    ~Decoder ();
    ...
    PP_TimeTicks GetAverageLatency() {return num_pictures_ ? total_latency_ / num_pictures_ : 0;}
...
private:
    ...
    // VP8 is stored in an IVF container. Helpful description: http://wiki.multimedia.cx/index.php?title=IVF
...
Decoder::Decoder (SweetDecodeInstance* instance, int id, const pp::Graphics3D& graphics_3d)
    : instance_         (instance),
      id_               (id),
...
void Decoder::Start ()
{
    ...
    decoder_->GetPicture(callback_factory_.NewCallbackWithOutput(&Decoder::PictureReady));
...
void Decoder::PictureReady (int32_t result, PP_VideoPicture picture) 
{
    ...
    num_pictures_++;
    PP_TimeTicks latency = core_if_->GetTimeTicks() - decode_time_[picture.decode_id % kMaxDecodeDelay];
...
SweetDecodeInstance::SweetDecodeInstance (PP_Instance instance, pp::Module* module)
...
SweetDecodeInstance::~SweetDecodeInstance () 
{
    if (!context_) 
    {
        return;
    ...
    for (DecoderList::iterator it = video_decoders_.begin(); it != video_decoders_.end(); ++it) 
    {
        delete *it;
    }
...
void SweetDecodeInstance::DidChangeView(const pp::Rect& position, const pp::Rect& clip_ignored)
{
...
bool SweetDecodeInstance::HandleInputEvent (const pp::InputEvent& event) 
{
...
void SweetDecodeInstance::InitializeDecoders () 
{
...
void SweetDecodeInstance::PaintPicture (Decoder* decoder, const PP_VideoPicture& picture) 
{
...
void SweetDecodeInstance::PaintNextPicture () // This is the heart of painting the image // Open GL Wise.
{
    assert(!is_painting_);
    is_painting_ = true;
    ...
    context_->SwapBuffers(callback_factory_.NewCallback(&SweetDecodeInstance::PaintFinished));
}

void SweetDecodeInstance::PaintFinished (int32_t result) 
{
    assert(result == PP_OK);
...
void SweetDecodeInstance::InitGL () 
{
  ...
  SWEET_ASSERT_GL_THROW(gles2_if_, context_);

  CreateGLObjects();
}

void SweetDecodeInstance::CreateGLObjects () 
{
...
void SweetDecodeInstance::Create2DProgramOnce () 
{
    if (shader_2d_.program)
    {
        return;
...
void SweetDecodeInstance::CreateRectangleARBProgramOnce () 
{
    if (shader_rectangle_arb_.program)
    {
        return;
    ...
    shader_rectangle_arb_ = CreateProgram(kVertexShader, kFragmentShaderRectangle);

    SWEET_ASSERT_GL_THROW(gles2_if_, context_);
}

void SweetDecodeInstance::CreateExternalOESProgramOnce () 
{
    if (shader_external_oes_.program)
    {
        return;
...
Shader SweetDecodeInstance::CreateProgram (const char* vertex_shader, const char* fragment_shader) 
{
    Shader shader;

    // Create shader program.
    shader.program = gles2_if_->CreateProgram(context_->pp_resource()); // HERE.
    
    CreateShader(shader.program, GL_VERTEX_SHADER , vertex_shader ,static_cast<uint32_t>(strlen(vertex_shader)));
    CreateShader(shader.program,GL_FRAGMENT_SHADER,fragment_shader,static_cast<int>(strlen(fragment_shader)));

    gles2_if_->LinkProgram(context_->pp_resource(), shader.program);
    gles2_if_->UseProgram(context_->pp_resource(), shader.program);
    
    gles2_if_->Uniform1i(context_->pp_resource(),
                         gles2_if_->GetUniformLocation(context_->pp_resource(), shader.program, "s_texture"),
                         0);
...
void SweetDecodeInstance::CreateShader (GLuint program, GLenum type, const char* source, int size) 
{
...
// as it is loaded.
class SweetDecoderModule : public pp::Module 
{
public:
             SweetDecoderModule () : pp::Module() {}
    virtual ~SweetDecoderModule () {}

    virtual pp::Instance* CreateInstance (PP_Instance instance) {return new SweetDecodeInstance(instance, this);}
};
