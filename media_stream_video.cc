...
void MediaStreamVideoDemoInstance::OnGetFrame(int32_t result, pp::VideoFrame frame) // Here is some important stuff.
{
  if (result != PP_OK)
    return;
    ...
    if (!is_bgra_) 
    {
        ...
        std::stringstream ss;
        ss << "/Users/james/Desktop/yuv/yuv-" << frame_size_.width() << "x" << frame_size_.height() << "-" << ++i << ".data";
